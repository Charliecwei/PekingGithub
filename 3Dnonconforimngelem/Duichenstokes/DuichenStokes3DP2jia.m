...
%  bdFlag = setboundary3(node,elem,'Dirichlet','all','Neumann','(z==1)&(z==-1)');
   bdFlag = setboundary3(node,elem,'Dirichlet','all','Neumann','(0<y&y<1)&(0<x&x<1)&(z==1)');
    
    % solve the equation
   [soln,eqn] = DuichenStoke3P2jia3B3andP1(node,elem,bdFlag,pde);
    ...
    h(k) = 2./(power(size(node,1),1/3)-1);
     ...
     errp(k) = getL2error3D(node,elem,pde.exactp,ph,'P1');
     errL2u(k) = getstokeserrL2u3D(node,elem,pde.exactu,uh,'P2jia');
     errH1u(k) = getstkoeserrH1u3D(node,elem,pde.exactDu,uh,'P2jia');
...
function uI = chazhi(Nu,exactu,node,face,elem)
     ...
     for p=1:nQuad
            pxyz=lambda(p,1)*node(face(:,1),:)+lambda(p,2)*node(face(:,2),:)...
             ...
             u1(1:NQ)=u1(1:NQ)+w(p)*[uD1*lambda(p,1);uD1*lambda(p,2);uD1*lambda(p,3)];
             u2(1:NQ)=u2(1:NQ)+w(p)*[uD2*lambda(p,1);uD2*lambda(p,2);uD2*lambda(p,3)];
             u3(1:NQ)=u3(1:NQ)+w(p)*[uD3*lambda(p,1);uD3*lambda(p,2);uD3*lambda(p,3)];
