function [soln,eqn,info] = Poisson3DP2N(node,elem,bdFlag,pde,option,varargin)
%% POISSON3DP2N Poisson equation: P2 quadratic nonconforming element in 3-D
%
...
[elem2dof,edge] = dof3DP2N(elem);
Ndof = N + size(edge,1) + NT;
...
if isPureNeumann
    ...
    [lambda,w] = quadpts3(3);  %This is P3 element.
    nQuad = size(lambda,1);
    phi = getphi(lambda);
    UD = u(elem2dof);
    for  p = 1:nQuad
...
%% Boundary conditions for Poisson equation: P2 quadratic FEM in 3-D.
    ...
    if ~isempty(bdFlag)
        % Find boundary edges and nodes
        bdFace2dof = [elem2dof(bdFlag(:,1) == 2,[2,3,4,10,9,8,11]); ...  
                      elem2dof(bdFlag(:,2) == 2,[1,4,3,10,6,7,11]); ...
                      elem2dof(bdFlag(:,3) == 2,[1,2,4,9,7,5,11]); ...
                      elem2dof(bdFlag(:,4) == 2,[1,3,2,8,5,6,11])];
    end
    ...
    if ~isempty(bdFace2dof) && ~isempty(pde.g_N) && ~(isnumeric(pde.g_N) && (pde.g_N == 0))
        ...
        phigN(:,7) = 2 - 4*(lambdagN(:,1).^2+lambdagN(:,2).^2+lambdagN(:,3).^2);
        nQuadgN = size(lambdagN,1);
        gf = zeros(size(bdFace2dof,1),7);
        for pp = 1:nQuadgN
