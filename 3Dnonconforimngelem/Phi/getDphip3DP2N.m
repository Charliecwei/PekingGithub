function Dphip = getDphip3DP2N(Dlambda,lambda)
    ...
    Dphip(:,:,11) = -8*(lambda(p,1)*Dlambda(:,:,1)+lambda(p,2)*Dlambda(:,:,2)...
                                           +lambda(p,3)*Dlambda(:,:,3)+lambda(p,4)*Dlambda(:,:,4)); 
%    Dphip(:,:,11) = Dlambda(:,:,1)*lambda(p,2)*lambda(p,3)*lambda(p,4)...          
%                               +Dlambda(:,:,2)*lambda(p,1)*lambda(p,3)*lambda(p,4)...   
%                               +Dlambda(:,:,3)*lambda(p,2)*lambda(p,1)*lambda(p,4)...   
%                               +Dlambda(:,:,4)*lambda(p,2)*lambda(p,3)*lambda(p,1);
%                       
