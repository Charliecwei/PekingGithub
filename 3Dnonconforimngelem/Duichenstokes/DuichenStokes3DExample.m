function DuichenStokes3DExample
%%求解对称sotkes方程
% 
...
% usolver: 3DP2   3DP2N    P2jia   3DP3    3DP3N     P3jia5B4  3DP3jia8B4  
% psolver: P1 P2
%option = struct('usolver','3DP2','psolver','P1');
%option = struct('usolver','3DP2N','psolver','P1');
%option = struct('usolver','P2jia','psolver','P1');
% option = struct('usolver','3DP3','psolver','P2');
% option = struct('usolver','3DP3N','psolver','P2');
 option = struct('usolver','3DP3jia8B4','psolver','P2');

%% Generate an initial mesh 
%[node,elem] = cubemesh([0,1,0,1,0,1],1);
[node,elem] = cubemesh([-1,1,-1,1,-1,1],2); % data大于等于6时

%% PDE and options
pde = Duichenstokes3data6;
%pde = stokes3data5;
...
%  bdFlag = setboundary3(node,elem,'Dirichlet');
   bdFlag = setboundary3(node,elem,'Dirichlet','all','Neumann','(z==1)&(x==-1)');
...
%   bdFlag = setboundary3(node,elem,'Dirichlet','all','Neumann','z==1'); 
    ...
    % solve the equation
   [soln,eqn] = DuichenStoke3D(node,elem,bdFlag,pde,option);
    ...
    h(k) = 2./(power(size(node,1),1/3)-1);     
     ...
     errp(k) = getL2error3D(node,elem,pde.exactp,ph,option.psolver);
     errL2u(k) = getstokeserrL2u3D(node,elem,pde.exactu,uh,option.usolver);
     errH1u(k) = getstkoeserrH1u3D(node,elem,pde.exactDu,uh,option.usolver);
