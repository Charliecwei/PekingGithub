function  [soln,eqn] = Poisson3P3jia8B4(node,elem,bdFlag,pde,option)
%%P3jia8B4非协调元求解Poisson方程
% 
...
[elem3dof3,face] = dof3P3jia8B4s(elem);
%Nd = size(node,1);
NT = size(elem,1);  
NF = size(face,1);
...
ii = zeros(406*NT,1); jj = zeros(406*NT,1); 
index = 0;
for i=1:28
    for j=i:28
...
%% Boundary Conditions
[AD,u,ufreeDof,b,isPureNeumann] = getbdPossionP3jia8B4(node,face,bdFlag,b,...
...
eqn = struct('A',AD,'Lap',A,'f',b,...
    ...
    function [AD,u,ufreeDof,b,isPureNeumann] = getbdPossionP3jia8B4(node,face,bdFlag,b,...
                                                                                      Nu,elem3dof3,A,pde,elem,NF)
