...
bdFlag = setboundary(node,elem,'Neumann','all','Dirichlet','x==1|y==1');
...
for k=1:maxIt
    ...
    % solve the equation
    [soln,eqn] = Poisson2DP2N(node,elem,bdFlag,pde);
 % [soln,eqn] = Poisson3P3jia5P4(node,elem,bdFlag,pde);
    uh = soln.u;
    N(k) = length(uh);
    h(k) = 1./(size(node,1)^(1/2)-1);    

    errH1(k) = getH1error3D(node,elem,pde.exactDu,uh,'2DP2N');
    errL2(k) = getL2error3D(node,elem,pde.exactu,uh,'2DP2N');    
