function  [soln,eqn] = DuichenStoke3P3jia5B4andP2(node,elem,bdFlag,pde,option)
...
[elem3dofp2,edge] = dof3P2(elem); 
N = size(node,1);  NT = size(elem,1);NE = size(edge,1);
Nu = 6*size(face,1) + NT;   Np = N + NE;
...
B=divergence(Np,Nu,lambda,Dlambda,w,elem3dofp2,volume,elem3dof3);

%% Assemble right hand side
[f1,f2,f3]=righthand(Nu,option,pde,NT,node,elem,elem3dof3,volume);

%% Boundary Conditions
[AD,BD,f,u,p,g,ufreeDof,pDof] = getbdStokesP3jia5B4(node,face,bdFlag,f1,f2,f3,...
                                                                                      Np,Nu,elem3dof3,A,B,pde,elem);
...
info.assembleTime = assembleTime;
    ...
    function [AD,BD,f,u,po,g,ufreeDof,pDof] = getbdStokesP3jia5B4(node,face,bdFlag,f1,f2,f3,...
                                                                                      Np,Nu,elem3dof3,A,B,pde,elem)
           ...
           if ~isempty(Neumannidx)
               ...
               for pp = 1:nQuad
                   ...
                   g1 = w(pp)*[areafaces.*gp(:,1).*bdphiface(:,1);areafaces.*gp(:,1).*bdphiface(:,2); ...
                                        ...
                                        areafaces.*gp(:,1).*bdphiface(:,13);areafaces.*gp(:,1).*bdphiface(:,14);...
                                         areafaces.*gp(:,1).*bdphiface(:,15);areafaces.*gp(:,1).*bdphiface(:,16);...
                                         areafaces.*gp(:,1).*bdphiface(:,17);areafaces.*gp(:,1).*bdphiface(:,18);...
                                        areafaces.*gp(:,1).*bdphiface(:,19);areafaces.*gp(:,1).*bdphiface(:,20);...
                                        areafaces.*gp(:,1).*bdphiface(:,21);areafaces.*gp(:,1).*bdphiface(:,22);...
                                        areafaces.*gp(:,1).*bdphiface(:,23);areafaces.*gp(:,1).*bdphiface(:,24);...
                                         areafaces.*gp(:,1).*bdphiface(:,25) ];
                   ...
                   g2 =  w(pp)*[areafaces.*gp(:,2).*bdphiface(:,1);areafaces.*gp(:,2).*bdphiface(:,2); ...
                                          ...
                                          areafaces.*gp(:,2).*bdphiface(:,13);areafaces.*gp(:,2).*bdphiface(:,14);...
                                          areafaces.*gp(:,2).*bdphiface(:,15);areafaces.*gp(:,2).*bdphiface(:,16);...
                                          areafaces.*gp(:,2).*bdphiface(:,17);areafaces.*gp(:,2).*bdphiface(:,18);...
                                          areafaces.*gp(:,2).*bdphiface(:,19);areafaces.*gp(:,2).*bdphiface(:,20);...
                                          areafaces.*gp(:,2).*bdphiface(:,21);areafaces.*gp(:,2).*bdphiface(:,22);...
                                          areafaces.*gp(:,2).*bdphiface(:,23);areafaces.*gp(:,2).*bdphiface(:,24);...
                                          areafaces.*gp(:,2).*bdphiface(:,25)];
                   ...
                   g3 =  w(pp)*[areafaces.*gp(:,3).*bdphiface(:,1);areafaces.*gp(:,3).*bdphiface(:,2); ...
                                          ...
                                          areafaces.*gp(:,3).*bdphiface(:,13);areafaces.*gp(:,3).*bdphiface(:,14);...
                                          areafaces.*gp(:,3).*bdphiface(:,15);areafaces.*gp(:,3).*bdphiface(:,16);...
                                          areafaces.*gp(:,3).*bdphiface(:,17);areafaces.*gp(:,3).*bdphiface(:,18);...
                                          areafaces.*gp(:,3).*bdphiface(:,19);areafaces.*gp(:,3).*bdphiface(:,20);...
                                          areafaces.*gp(:,3).*bdphiface(:,21);areafaces.*gp(:,3).*bdphiface(:,22);...
                                          areafaces.*gp(:,3).*bdphiface(:,23);areafaces.*gp(:,3).*bdphiface(:,24);...
                                          areafaces.*gp(:,3).*bdphiface(:,25)];
...
nQuad = size(lambda,1);
sA11=zeros(325*NT,nQuad);
sA12=zeros(325*NT,nQuad);
sA13=zeros(325*NT,nQuad);
sA21=zeros(325*NT,nQuad);
sA22=zeros(325*NT,nQuad);
sA23=zeros(325*NT,nQuad);
sA31=zeros(325*NT,nQuad);
sA32=zeros(325*NT,nQuad);
sA33=zeros(325*NT,nQuad);
...
function B=divergence(Np,Nu,lambda,Dlambda,w,elem3dofp2,volume,elem3dof3)
%%计算散度项B
Dx = sparse(Np,Nu);
Dy = sparse(Np,Nu);
Dz = sparse(Np,Nu);
   ...
   phipp = getphip2(lambda);
       ...
       for i = 1:25 
             for j = 1:10
                   ...
                   Dx = Dx + sparse(double(elem3dofp2(:,j)),double(elem3dof3(:,i)),Dxij.*volume,Np,Nu);
                   Dy = Dy + sparse(double(elem3dofp2(:,j)),double(elem3dof3(:,i)),Dyij.*volume,Np,Nu);
                   Dz = Dz + sparse(double(elem3dofp2(:,j)),double(elem3dof3(:,i)),Dzij.*volume,Np,Nu);
...
function [f1,f2,f3]=righthand(Nu,option,pde,NT,node,elem,elem3dof3,volume)
%% 计算右端项
f1 = zeros(Nu,1);
f2 = zeros(Nu,1);
f3 = zeros(Nu,1);
...
if ~isempty(pde.f) 
    ...
    ft1 = ft1.*repmat(volume,1,25);
    ft2 = ft2.*repmat(volume,1,25);
    ft3 = ft3.*repmat(volume,1,25);
    f1 = accumarray(elem3dof3(:),ft1(:),[Nu 1]);
    f2 = accumarray(elem3dof3(:),ft2(:),[Nu 1]);
    f3 = accumarray(elem3dof3(:),ft3(:),[Nu 1]);
end
end
